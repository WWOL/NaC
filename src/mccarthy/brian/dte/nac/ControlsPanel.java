...
public class ControlsPanel extends JPanel implements ActionListener {
	...
	private JComboBox<GameLogic> combo = null;
	...
	private JComboBox<GameLogic> setupLogicComboBox() {
		combo = new JComboBox<GameLogic>();
		combo.setLocation(10, 240);
		combo.setSize(180, 25);
		combo.setSelectedItem(GameRunner.getGameBoard().getLogicHumanName());
		combo.addItemListener(new ItemListener() {
			@Override
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					// Change the logic type
					GameRunner.getGameBoard().setLogicType((GameLogic) e.getItem());
				}
			}
		});
		return combo;
	...
	private void setupLogicSelectorOptions() {
		...
		for (GameLogic logic : logicList) {
			System.out.println("Added " + logic.getHumanName() + " to the logic selector.");
			combo.addItem(logic);
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("CONTROLS_RESET")) {
			...
			JOptionPane.showMessageDialog(null, GameRunner.getHelpText() + GameRunner.getAboutText() + "Logic: " + GameRunner.getGameBoard().getLogicHumanName(), "Help / About", JOptionPane.PLAIN_MESSAGE);
